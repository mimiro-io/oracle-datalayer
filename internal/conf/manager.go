...
func (conf *ConfigurationManager) setUser(config *Datalayer) *Datalayer {
	...
	if config.User == "" {
		...
		if userName == "" {
			conf.logger.Warn("No user name defined: set env var: *go_ora.Connection_DB_USER. Or include in config.json")
		}
	...
	if config.Password == "" {
		...
		if password == "" {
			conf.logger.Warn("No password defined: set env var: DB_PASSWORD")
		}
